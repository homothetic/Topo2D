...
if __name__ == '__main__':
    ...
    # merge
    mmcv.mkdir_or_exist(os.path.join(data_root, 'data_splits'))
    merge_annotations(os.path.join(data_root, 'lane3d_1000', 'training'), os.path.join(data_root, 'data_splits', 'training.json'))
    merge_annotations(os.path.join(data_root, 'lane3d_1000', 'validation'), os.path.join(data_root, 'data_splits', 'validation.json'))
    # generate
    data_list_path = os.path.join(data_root, 'data_lists')
    os.makedirs(data_list_path, exist_ok=True)
    ori_json = os.path.join(data_root, 'data_splits', 'training.json')
    tar_path = os.path.join(data_root, 'cache_dense')
    extract_data_with_smoothing(data_root, ori_json, tar_path=tar_path, test_mode=False)
    generate_datalist(os.path.join(tar_path, 'training'), os.path.join(data_list_path, 'training.txt'))
    ori_json = os.path.join(data_root, 'data_splits', 'validation.json')
    tar_path = os.path.join(data_root, 'cache_dense')
    extract_data_with_smoothing(data_root, ori_json, tar_path=tar_path, test_mode=True)
