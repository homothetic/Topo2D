...
class CustomLoadMultiViewImageFromFilesOpenLaneV2(LoadMultiViewImageFromFiles):
...
@PIPELINES.register_module()
class CustomLoadMultiViewImageFromFilesPad(object):
    ...
    def __call__(self, results):
        ...
        results['img_shape'] = [(max_h, max_w, 3) for _ in img_list]
        results['ori_shape'] = img_shape_list
        results['pad_shape'] = [(max_h, max_w, 3) for _ in img_list]

        # img = np.stack(img_list, axis=-1)
        # if self.to_float32:
        #     img = img.astype(np.float32)
        # results['img'] = [img[..., i] for i in range(img.shape[-1])]
        results['img'] = [img.astype(np.float32) for img in img_list]
