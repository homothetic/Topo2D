...
def evaluate_centerline(ground_truth, predictions, topo=False, verbose=True):
    gts = {}
    preds = {}
    for token in ground_truth.keys():
        gts[token] = ground_truth[token]
        preds[token] = predictions['results'][token]['predictions']
    ...
    for token in tqdm(gts.keys(), desc='calculating distances:', ncols=80, disable=not verbose):
        ...
        mask = distance_matrixs['chamfer'][token] < THRESHOLDS_FRECHET[-1]
