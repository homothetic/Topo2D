...
def normalize_3d_pts(pts, pc_range):
    patch_h = pc_range[4]-pc_range[1] # 50, y
    patch_w = pc_range[3]-pc_range[0] # 100, x
    patch_z = pc_range[5]-pc_range[2] # 5, z
    new_pts = pts.clone()
...
class Topo2DAssigner3D(BaseAssigner):
    ...
    def assign(self,
        ...
        if gt_pts.shape[-1] == 2:
            normalized_gt_pts = normalize_2d_pts(gt_pts, self.pc_range) # x & y
        else:
            normalized_gt_pts = normalize_3d_pts(gt_pts, self.pc_range) # x & y & z
