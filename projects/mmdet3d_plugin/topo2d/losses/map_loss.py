...
@MATCH_COST.register_module()
class OrderedPtsL1CostInFOV(object):
    """OrderedPtsL1Cost.
     Args:
         weight (int | float, optional): loss_weight
    """

    def __init__(self, weight=1., mask=True):
        self.weight = weight
        self.mask = mask

    # mask is None for 2d lane det
    # mask is gt vis for 3d lane det
    def __call__(self, bbox_pred, gt_bboxes, mask=None):
        """
        Args:
            bbox_pred (Tensor): Predicted boxes with normalized coordinates
                (x, y), which are all in range [0, 1]. Shape
        ...
        if mask is None:
            num_gts, num_orders, num_pts, num_coords = gt_bboxes.shape
            mask = ((gt_bboxes > 0) & (gt_bboxes < 1)).all(-1) # 2, 2, 20
            mask = mask[..., None].repeat(1, 1, 1, 2) # 2, 2, 20, 2
            mask = mask.view(num_gts * num_orders, -1) # 2*2, 20*2
        else:
            # num_orders is 1
            num_gts, num_orders, num_pts, num_coords = gt_bboxes.shape
            mask = (mask > 0.5)[..., None].repeat(1, 1, 2) # 2, 20, 2
            mask = mask.view(num_gts * num_orders, -1) # 2*2, 20*2

        gt_bboxes = gt_bboxes.view(num_gts * num_orders, -1) # 2*2, 40
        bbox_pred = bbox_pred.view(bbox_pred.size(0), -1) # 10, 40
        dist = bbox_pred[:, None] - gt_bboxes[None] # 10, 2*2, 40
        
        bbox_cost = (dist * mask[None]).norm(dim=-1, p=1)
        return bbox_cost * self.weight
