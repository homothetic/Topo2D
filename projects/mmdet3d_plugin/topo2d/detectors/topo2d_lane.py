...
class Topo2DLane(MVXTwoStageDetector):
    """MapTR.
    Args:
    ...
    def __init__(self,
        ...
        if not self.only_2d:
            ...
            # self.query_embedding_2d = nn.Sequential( # 128 * 2 -> 256
    ...
    def forward_pts_train_3d(self, outs, pts_feats, gt_camera_extrinsic,
        ...
        if self.mask_head:
            if len(pts_feats) > 1:
                outputs_mask, outputs_scores = self.sparse_int(pts_feats[1], query_embeds_seg)
            else:
        ...
        def pad_tensor(t, N):
            if t.shape[0] < N:
                padding = t.new_zeros([N - t.shape[0], *t.shape[1:]])
                t = torch.cat((t, padding), dim=0)
            return t

        if self.nms_2d_proposal and self.topk_2d_proposal > 0:
            # [NOTE] only support nms now, do not use topk
            # if self.nms_2d_proposal:
            #     indexs = self.get_proposal_nms(outs, topk_num=self.topk_2d_proposal)
            indexs = self.get_proposal_nms(outs, 
                                            topk_num=self.topk_2d_proposal,
                                            nms_thresh=self.proposal_cfg_nms_thresh,
                                            score_thresh=self.proposal_cfg_score_thresh,
                                        )
            # else:
            #     indexs = self.get_proposal(outs, 
            #                                topk_num=self.topk_2d_proposal,
            #                                score_thresh=self.proposal_cfg_score_thresh,
            #                             )
            # print(indexs)
            proposal_list_topk = [] # B, N, 20, 2
            query_embeds_topk = [] # B, N, 256
            reference_points_topk = [] # B, N, 20, 64 * 3
            pos_embeds_topk = [] # B, N, 256
            vis_scores_topk = [] # B, N, 20, 1
            outputs_mask_topk = [] # B, N, H, W
            for i in range(len(indexs)):
                proposal_list_topk.append(pad_tensor(proposal_list[i][indexs[i]], self.topk_2d_proposal))
                query_embeds_topk.append(pad_tensor(query_embeds[i][indexs[i]], self.topk_2d_proposal))
                reference_points_topk.append(pad_tensor(reference_points[i][indexs[i]], self.topk_2d_proposal))
                pos_embeds_topk.append(pad_tensor(pos_embeds[i][indexs[i]], self.topk_2d_proposal))
                vis_scores_topk.append(pad_tensor(vis_scores[i][indexs[i]], self.topk_2d_proposal))
                outputs_mask_topk.append(pad_tensor(outputs_mask[i][indexs[i]], self.topk_2d_proposal))
            proposal_list = torch.stack(proposal_list_topk, dim=0)
            query_embeds = torch.stack(query_embeds_topk, dim=0)
            reference_points = torch.stack(reference_points_topk, dim=0)
            pos_embeds = torch.stack(pos_embeds_topk, dim=0)
            vis_scores = torch.stack(vis_scores_topk, dim=0)
            outputs_mask = torch.stack(outputs_mask_topk, dim=0)
    ...
    def forward_pts_train(self,
        ...
        if self.keep_assign:
            outs_3d = self.forward_pts_train_3d(outs, pts_feats, gt_camera_extrinsic,
                            gt_camera_intrinsic, gt_homography_matrix,
                            gt_project_matrix, img_metas)
            pos_inds_list = [None for _ in range(self.pts_bbox_head.transformer.decoder.num_layers)]
            pos_assigned_gt_inds = [None for _ in range(self.pts_bbox_head.transformer.decoder.num_layers)]
            num_total_pos = [None for _ in range(self.pts_bbox_head.transformer.decoder.num_layers)]
            losses, pos_inds_list, pos_assigned_gt_inds, num_total_pos = self.pts_bbox_head.loss(
                *loss_inputs, pos_inds_list, pos_assigned_gt_inds, num_total_pos, img_metas=img_metas)
            # losses, pos_inds_list, pos_assigned_gt_inds, num_total_pos = self.pts_bbox_head_3d.loss(
            #     gt_3dlanes, outs_3d, img_metas=img_metas)
            if self.mask_head:
                loss = self.sparse_int.loss(outs_3d['pred_masks'], outs_3d['pred_scores'], 
                                            seg_idx_label, pos_inds_list, pos_assigned_gt_inds, 
                                            num_total_pos)
                losses.update(loss)
            loss, _, _, _ = self.pts_bbox_head_3d.loss(gt_3dlanes, outs_3d, 
                pos_inds_list, pos_assigned_gt_inds, num_total_pos, img_metas=img_metas)
            # pos_inds_list = [None for _ in range(self.pts_bbox_head.transformer.decoder.num_layers)]
            # pos_assigned_gt_inds = [None for _ in range(self.pts_bbox_head.transformer.decoder.num_layers)]
            # num_total_pos = [None for _ in range(self.pts_bbox_head.transformer.decoder.num_layers)]
            # loss, _, _, _ = self.pts_bbox_head.loss(*loss_inputs, pos_inds_list, pos_assigned_gt_inds, 
            #                                         num_total_pos, img_metas=img_metas)
            losses.update(loss)
        else:
            pos_inds_list = [None for _ in range(self.pts_bbox_head.transformer.decoder.num_layers)]
            pos_assigned_gt_inds = [None for _ in range(self.pts_bbox_head.transformer.decoder.num_layers)]
            ...
            if not self.only_2d:
                outs_3d = self.forward_pts_train_3d(outs, pts_feats, gt_camera_extrinsic,
                                ...
                                gt_project_matrix, img_metas)
                if gt_labels_3d:
                    loss, _, _, _ = self.pts_bbox_head_3d.loss(gt_3dlanes, outs_3d, gt_labels_3d, img_metas=img_metas)
                else:
                    loss, _, _, _ = self.pts_bbox_head_3d.loss(gt_3dlanes, outs_3d, img_metas=img_metas)
                losses.update(loss)
                if self.mask_head:
                    loss = self.sparse_int.loss(outs_3d['pred_masks'], outs_3d['pred_scores'], 
                                                seg_idx_label, pos_inds_list, pos_assigned_gt_inds, 
    ...
    def pred2result(self, bboxes, scores, labels, pts, vis, attrs=None):
    ...
    def simple_test_pts(self, x, lidar_feat, img_metas, 
        ...
        if not self.only_2d:
            ...
            if 'attn_map' in outs_3d.keys() and self.vis_attn_map:
                attn_map = {
                    'mask': outs_3d['pred_masks'], # 1, 50, 60, 100, w/o sigmoid
                    'iou': outs_3d['pred_scores'], # 1, 50, 1, w/o sigmoid
                    'attn': outs_3d['attn_map'], 
                    'score':outs_3d['all_cls_scores'],
                }
            else:
                attn_map = None
            bbox_list = self.pts_bbox_head_3d.get_bboxes(outs_3d, img_metas, rescale=rescale)
            bbox_results = [
                self.pred2result(bboxes, scores, labels, pts, vis)
                for bboxes, scores, labels, pts, vis in bbox_list
            ]
            return None, bbox_results, attn_map
    ...
    def simple_test(self, img_metas, img=None, points=None, prev_bev=None, rescale=False, **kwargs):
        ...
        # import pdb; pdb.set_trace()
        # self.vis_pred_result(dataset='openlanev2', mode='2d', 
