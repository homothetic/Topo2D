...
class FlashAttention(nn.Module):
    ...
    def forward(self, q, kv, 
        ...
        if key_padding_mask is None:
            ...
            kv = kv.masked_fill(key_padding_mask.flatten()[..., None, None, None], 0.0)
            output = flash_attn_unpadded_kvpacked_func(
                q, kv, cu_seqlens_q, cu_seqlens_k, max_sq, max_sk,
                self.dropout_p if self.training else 0.0,
                softmax_scale=self.softmax_scale, causal=causal
        ...
        #     q, kv = rearrange(q, 'b s ... -> (b s) ...'), rearrange(kv, 'b s ... -> (b s) ...')
        #     # max_sk, cu_seqlens_k
        #     max_sq, max_sk = seqlen_q, len(key_padding_mask[0]) - key_padding_mask[0].sum().item()
        #     cu_seqlens_q = torch.arange(0, (batch_size + 1) * max_sq, step=max_sq, dtype=torch.int32,
        #                             device=q.device)
        #     cu_seqlens_k = torch.arange(0, (batch_size + 1) * max_sk, step=max_sk, dtype=torch.int32,
        #                             device=kv.device)
        #     # kv
        #     indices = torch.where(key_padding_mask.flatten() == False)[0]
        #     kv = kv[indices]
        #     output = flash_attn_unpadded_kvpacked_func(
        #         q, kv, cu_seqlens_q, cu_seqlens_k, max_sq, max_sk,
        #         self.dropout_p if self.training else 0.0,
        ...
        #     output = rearrange(output, '(b s) ... -> b s ...', b=batch_size)
        # else:
        #     # [TODO] still bugs, so do not use this section
        #     nheads = kv.shape[-2]
        ...
        #     # [BUG] key_padding_mask 1 means valid and 0 means not valid
...
if __name__ == '__main__':
    ...
    k = torch.randn(batch_size, 1024, n, device='cuda', dtype=dtype, requires_grad=True)
    v = torch.randn(batch_size, 1024, n, device='cuda', dtype=dtype, requires_grad=True)
    key_padding_mask = k.new_ones(batch_size, 1024)
    
    model = FlashMHA(n, nheads, device=device, attention_dropout=dropout_p, dtype=dtype)
    model.to(dtype).to(device)
    model.eval()
    
    model2 = nn.MultiheadAttention(n, nheads, device=device, dropout=dropout_p, dtype=dtype)
    model2.to(dtype).to(device)
    model2.eval()
    ...
    out1 = model(q, k, v, key_padding_mask)[0]
    out2 = model2(q.transpose(1, 0), k.transpose(1, 0), v.transpose(1, 0), key_padding_mask)[0].transpose(1, 0)
