...
class Topo2DHead2DLane(DETRHead):
    """Head of Detr3D.
    Args:
    ...
    def __init__(self,
        ...
        super(Topo2DHead2DLane, self).__init__(
    ...
    def loss_single(self,
        ...
        loss_vis = torch.zeros_like(loss_bbox)
        if self.output_vis:
    ...
    def loss(self,
        ...
        all_cls_scores = preds_dicts['all_cls_scores'] # [24, 50, 1] * 6
        all_bbox_preds = preds_dicts['all_bbox_preds'] # [24, 50, 4] * 6
        all_pts_preds  = preds_dicts['all_pts_preds'] # [24, 50, 20, 2] * 6
        all_vis_scores = preds_dicts['all_vis_scores'] # [24, 50, 20] * 6
        # enc_cls_scores = preds_dicts['enc_cls_scores'] # None
        # enc_bbox_preds = preds_dicts['enc_bbox_preds'] # None
        # enc_pts_preds  = preds_dicts['enc_pts_preds'] # None

        self.pos_inds_list = pos_inds_list[-1] if pos_inds_list else None
        self.pos_assigned_gt_inds = pos_assigned_gt_inds[-1] if pos_assigned_gt_inds else None
        self.num_total_pos = num_total_pos[-1] if num_total_pos else None
        ...
        cls_scores (Tensor): Box score logits from a single decoder layer 
            for all images. Shape [bs, num_query, cls_out_channels].
            shape [layer, batch, num_query, cls]. [TODO: w/o sigmoid]
        
        bbox_preds (Tensor): Sigmoid outputs from a single decoder layer 
            for all images, with normalized coordinate (cx, cy, w, h).
            shape [layer, batch, num_query, 4]. [TODO: w/ sigmoid]

        pts_preds (Tensor): Reference points from a single decoder layer 
            for all images, with normalized coordinate (cx, cy, w, h).
            shape [layer, batch, num_query, 20, 2].
        ...
        gt_bboxes_list = gt_2dboxes
        gt_labels_list = gt_labels
        # gt_pts_list = []
        gt_shifts_pts_list = gt_2dlanes

        # import pdb; pdb.set_trace()
        if len(gt_labels) != all_cls_scores.shape[1]: # check N == 1
            gt_bboxes_list = []
            gt_labels_list = []
            gt_shifts_pts_list = []
        ...
        # [NOTE] only support multi class openlane
        elif self.cls_out_channels > 1:
            # the same cls label as 3d decoder
            # import pdb; pdb.set_trace()
            gt_labels_list = []
        ...
        loss_dict['loss_vis'] = losses_vis[-1]
        ...
        for loss_cls_i, loss_vis_i, loss_bbox_i, loss_iou_i, loss_pts_i, loss_dir_i in zip(losses_cls[:-1],
            ...
            loss_dict[f'd{num_dec_layer}.loss_vis'] = loss_vis_i
