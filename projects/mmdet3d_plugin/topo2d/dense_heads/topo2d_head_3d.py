...
class Topo2DHead3D(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        super(Topo2DHead3D, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _init_layers(self):
        ...
        input_proj = Conv2d(self.in_channels, self.embed_dims, kernel_size=1)
    ...
    def position_embeding_norm(self, 
                          img_feats, 
                          img_metas, 
                          gt_project_matrix, 
                          position_level=0,
        ...
        B, N, C, H, W = img_feats[position_level].shape
        # [BUG] img_shape is (1550, 2048), pad_shape is (480, 800)
        # pad_h, pad_w = img_metas[0]['pad_shape'][0][0], img_metas[0]['pad_shape'][0][1]
        pad_h, pad_w = img_metas[0]['img_shape'][0][0], img_metas[0]['img_shape'][0][1]
        ...
        coords_d_x = self.depth_start + bin_size * index * index_1
        D = coords_d_x.shape[0]
        coords_x = torch.stack(torch.meshgrid([coords_w, coords_h, coords_d_x])).permute(1, 2, 3, 0) # W, H, D, 3
        coords_x = torch.cat((coords_x, torch.ones_like(coords_x[..., :1])), -1)
        coords_x[..., :2] = coords_x[..., :2] * torch.maximum(coords_x[..., 2:3], torch.ones_like(coords_x[..., 2:3]) * eps)
        coords_x = coords_x.view(1, 1, W, H, D, 4, 1).repeat(B, 1, 1, 1, 1, 1, 1)
        # y
        bin_size = (self.position_range[4] - self.depth_start) / (self.depth_num * (1 + self.depth_num))
        coords_d_y = self.depth_start + bin_size * index * index_1
        D = coords_d_y.shape[0]
        coords_y = torch.stack(torch.meshgrid([coords_w, coords_h, coords_d_y])).permute(1, 2, 3, 0) # W, H, D, 3
        coords_y = torch.cat((coords_y, torch.ones_like(coords_y[..., :1])), -1)
        coords_y[..., :2] = coords_y[..., :2] * torch.maximum(coords_y[..., 2:3], torch.ones_like(coords_y[..., 2:3]) * eps)
        coords_y = coords_y.view(1, 1, W, H, D, 4, 1).repeat(B, 1, 1, 1, 1, 1, 1)
        ...
        coords = torch.cat([
            coords_x, coords_y, coords_y, coords_x, coords_x, coords_y, coords_y,
        ], dim=1) # B, N, W, H, D, 4, 1
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is Topo2DHead3D:
            convert_dict = {
    ...
    def forward(self, 
        ...
        """
        # import pdb; pdb.set_trace()
        x_roi, pos_embed_roi, masks = [], [], []
        for lvl_idx in range(self.n_level):
            x = mlvl_feats[lvl_idx]
            batch_size, num_cams = x.size(0), x.size(1)
            
            x = self.input_proj_list[lvl_idx](x.flatten(0, 1))
            x = x.view(batch_size, num_cams, *x.shape[-3:])
            mask = x.new_zeros((batch_size, num_cams, *x.shape[-2:])).to(torch.bool) # B, cam, H, W
            ...
            if self.with_multiview:
                sin_embed = self.positional_encoding(mask)
                sin_embed = self.adapt_pos3d(sin_embed.flatten(0, 1)).view(x.size())
                pos_embed = pos_embed + sin_embed

            bs, n, c, h, w = x.shape
            x = x.permute(1, 3, 4, 0, 2).reshape(-1, bs, c) # [bs, n, c, h, w] -> [n*h*w, bs, c]
            pos_embed = pos_embed.permute(1, 3, 4, 0, 2).reshape(-1, bs, c) # [bs, n, c, h, w] -> [n*h*w, bs, c]
            pos_embed = pos_embed + self.level_embeds[lvl_idx].view(1, 1, -1)
            mask = mask.view(bs, -1)  # [bs, n, h, w] -> [bs, n*h*w]

            x_roi.append(x)
            pos_embed_roi.append(pos_embed)
            masks.append(mask)
        
        x_roi = torch.cat(x_roi, dim=0) # K, B, C
        pos_embed_roi = torch.cat(pos_embed_roi, dim=0) # K, B, C
        masks = torch.cat(masks, dim=1) # B, K
        
        # import pdb; pdb.set_trace()
        assert batch_size == 1 and num_cams == 7
        ...
        outs_dec, attn_map_for_vis_list, _ = self.transformer(x_roi, masks, 
                                                              target_one2many, query_embeds_one2many, 
                                                              pos_embed_roi, 
    ...
    def control_points_to_lane_points(self, lanes):
        ...
        bezier_A = torch.tensor(A, dtype=torch.float32).to(lanes.device) # pts, control
        lanes = torch.einsum('ij,bnjk->bnik', bezier_A, lanes)
    ...
    def loss_single(self,
        ...
        normalized_pts_targets = normalize_3d_pts(pts_targets, self.pc_range)
        ...
        denormed_pts_preds = denormalize_3d_pts(pts_preds, self.pc_range)
        ...
        loss_dir = self.loss_dir(
            denormed_pts_preds_dir, pts_targets_dir, dir_weights, avg_factor=num_total_pos)
    ...
    def loss(self,
        ...
        all_vis_scores = preds_dicts['all_vis_scores'][:, :, :self.num_lanes_one2one]
        ...
        all_vis_scores_one2many = preds_dicts['all_vis_scores'][:, :, self.num_lanes_one2one:]
        ...
        if gt_3dlanes[0].dim() == 2: # N, 605
            ...
            for idx, gt_3dlane in enumerate(gt_3dlanes):
                # N, 2, 20, 2
                gt_shifts_pts_list.append(gt_3dlane) # x & y & z attention
                if gt_labels_3d:
                    gt_labels_list.append(gt_labels_3d[idx])
                else:
        ...
        for gt_vis in gt_vis_list:
            if len(gt_vis):
                one2many_gt_vis_list.append(gt_vis.repeat(self.k_one2many, 1))
            else:
                one2many_gt_vis_list.append(gt_vis)
        ...
        all_gt_vis_list_one2many = [one2many_gt_vis_list for _ in range(num_dec_layers)]
        ...
        # import pdb;pdb.set_trace()
        losses_cls, _, _, _, losses_pts, _, pos_inds_list, pos_assigned_gt_inds, num_total_pos = multi_apply(
            self.loss_single, all_cls_scores, all_vis_scores, 
            all_bbox_preds, all_pts_preds,
            all_gt_bboxes_list, all_gt_labels_list, 
            all_gt_shifts_pts_list, all_gt_vis_list,
            all_gt_bboxes_ignore_list)
        
        losses_cls_one2many, _, _, _, losses_pts_one2many, _, _, _, _ = multi_apply(
            ...
            all_gt_shifts_pts_list_one2many, all_gt_vis_list_one2many,
        ...
        loss_dict['loss_3d_cls_h'] = losses_cls_one2many[-1] * self.lambda_one2many
        loss_dict['loss_3d_pts_h'] = losses_pts_one2many[-1] * self.lambda_one2many
        ...
        for loss_cls_i, loss_pts_i in zip(losses_cls[:-1], losses_pts[:-1]):
            ...
            loss_dict[f'd{num_dec_layer}.loss_3d_cls_h'] = losses_cls_one2many[num_dec_layer] * self.lambda_one2many
            loss_dict[f'd{num_dec_layer}.loss_3d_pts_h'] = losses_pts_one2many[num_dec_layer] * self.lambda_one2many
            num_dec_layer += 1
    ...
    @force_fp32(apply_to=('preds_dicts', 'lclc_preds_list'))
    def get_bboxes_topo(self, preds_dicts, lclc_preds_list, img_metas, rescale=False):
        """Generate bboxes from bbox head predictions.
        Args:
            preds_dicts (tuple[list[dict]]): Prediction results.
            img_metas (list[dict]): Point cloud and image's meta info.
        ...
        """
        # import pdb; pdb.set_trace()
        preds_dicts = self.bbox_coder.decode(preds_dicts, lclc_preds_list)
        ...
        for i in range(num_samples):
            ...
            pts = preds['pts']
            vis = preds['vis'] if 'vis' in preds.keys() else torch.ones_like(pts)
            topo = preds['topo_lclc']

            ret_list.append([bboxes, scores, labels, pts, vis, topo])

        return ret_list

    @force_fp32(apply_to=('preds_dicts', 'lclc_preds_list', 'lcte_preds_list'))
    def get_bboxes_topo_traffic(self, preds_dicts, lclc_preds_list, lcte_preds_list, 
        ...
        """
        # import pdb; pdb.set_trace()
        preds_dicts = self.bbox_coder.decode(preds_dicts, lclc_preds_list, lcte_preds_list)
        ...
        for i in range(num_samples):
            ...
            pts = preds['pts']
            vis = preds['vis'] if 'vis' in preds.keys() else torch.ones_like(pts)
            topo_lclc = preds['topo_lclc']
            topo_lcte = preds['topo_lcte']

            ret_list.append([bboxes, scores, labels, pts, vis, topo_lclc, topo_lcte])
